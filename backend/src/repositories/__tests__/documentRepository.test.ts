...
describe("documentRepository", () => {
  ...
  describe("findByHashAndFilename", () => {
    it("should find document by hash and filename", async () => {
      ...
      const result = await documentRepository.findByHashAndFilename(
        "abc123",
        "test.txt",
      );
  ...
  describe("create", () => {
    ...
    it("should handle creation errors", async () => {
      ...
      await expect(documentRepository.create(doc)).rejects.toThrow(
        "Creation failed",
      );
    });
  });
  ...
  describe("countByUserId", () => {
    it("should count active documents for user", async () => {
      dynamoMock.on(ScanCommand).resolves({
        Count: 5,
      });

      const count = await documentRepository.countByUserId("1");

      expect(count).toBe(5);
      expect(dynamoMock.calls()).toHaveLength(1);
      const call = dynamoMock.call(0);
      expect(call.args[0].input.FilterExpression).toContain("userId = :userId");
      expect(call.args[0].input.Select).toBe("COUNT");
    });
