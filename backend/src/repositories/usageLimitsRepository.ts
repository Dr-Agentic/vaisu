...
export class UsageLimitsRepository {
  ...
  async getUsageLimits(userId: string, period: string): Promise<UsageLimits | null> {
  ...
  async incrementDocumentCount(userId: string, amount: number = 1): Promise<UsageLimits> {
    const now = new Date();
    const period = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    ...
    const command = new UpdateCommand({
      TableName: TABLE_NAME,
      Key: { userId, period },
      UpdateExpression: 'SET documentCount = documentCount + :amount, updatedAt = :updatedAt',
  ...
  async incrementAnalysisCount(userId: string, amount: number = 1): Promise<UsageLimits> {
    const now = new Date();
    const period = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    ...
    const command = new UpdateCommand({
      TableName: TABLE_NAME,
      Key: { userId, period },
      UpdateExpression: 'SET analysisCount = analysisCount + :amount, updatedAt = :updatedAt',
      ExpressionAttributeValues: {
        ':amount': amount,
        ':updatedAt': now.toISOString(),
      },
  ...
  async incrementApiCalls(userId: string, amount: number = 1): Promise<UsageLimits> {
    const now = new Date();
    const period = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    ...
    const command = new UpdateCommand({
      TableName: TABLE_NAME,
      Key: { userId, period },
      UpdateExpression: 'SET apiCalls = apiCalls + :amount, updatedAt = :updatedAt',
      ExpressionAttributeValues: {
        ':amount': amount,
        ':updatedAt': now.toISOString(),
      },
  ...
  async incrementStorageUsed(userId: string, bytes: number): Promise<UsageLimits> {
    const now = new Date();
    const period = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    ...
    const command = new UpdateCommand({
      TableName: TABLE_NAME,
      Key: { userId, period },
      UpdateExpression: 'SET storageUsed = storageUsed + :bytes, updatedAt = :updatedAt',
      ExpressionAttributeValues: {
        ':bytes': bytes,
        ':updatedAt': now.toISOString(),
      },
  ...
  async checkLimits(userId: string, limits: UsageLimitConfig = this.defaultLimits): Promise<{ allowed: boolean; exceeded: string[] }> {
  ...
  async getUsageHistory(userId: string, limit: number = 12): Promise<UsageLimits[]> {
